...
export interface Point {
  ...
  startingTeam: 'us' | 'them'; // Who starts with possession (after pull)
  events: string[];    // Array of event IDs in order
  scoringTeam: 'us' | 'them' | null; // Null until point ends
  // Note: Current possession can be inferred from startingTeam + turnover events (flips on throwaway/drop/stall/block)
}